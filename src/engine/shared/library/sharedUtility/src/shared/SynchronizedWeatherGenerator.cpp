...
void SynchronizedWeatherGenerator::generateWeather(unsigned long serverTime) const
{
    ...
    m_weather.m_wind = t.rotate_l2p(Vector::unitZ * static_cast<float>(
        m_minWind + m_randomGenerator->random(m_maxWind - m_minWind)));

    m_weather.m_weatherIntensity = m_randomGenerator->random(3); // TODO: make higher intensities rarer

    int targetTemp = m_minTemp + m_randomGenerator->random(m_maxTemp - m_minTemp);
    float blend = 0.1f; // lower = slower transition, higher = faster
    m_weather.m_temperature = (m_weather.m_temperature * (1.0f - blend)) +
        (targetTemp * blend);
}
    ...
    m_randomGenerator(new RandomGenerator)
{
    ...
    // standardized mild climate (0–35°C, 0–10 m/s wind)
